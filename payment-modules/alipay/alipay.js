    ...
    alipayNotify.verifyNotify(_POST, function (verify_result) {
        if (verify_result) {//验证成功
            ...
            if (trade_status == 'TRADE_FINISHED') {
                ...
                console.log("trade_status == 'TRADE_SUCCESS':");
...
Alipay.prototype.create_direct_pay_by_user_return = function (req, res) {
    var self = this;
    var _GET = req.query;
    console.log("create_direct_pay_by_user_return开始？："+_GET);
    //计算得出通知验证结果
    var alipayNotify = new AlipayNotify(this.alipay_config);
    alipayNotify.verifyReturn(_GET, function (verify_result) {
        if (verify_result) {//验证成功
            ...
            console.log("var return_url = _GET['body'];"+return_url);
            return_url = new Buffer(return_url, 'base64').toString();
            console.log("new Buffer(return_url, 'base64').toString()"+return_url);
            //交易状态
            var trade_status = _GET['trade_status'];
            ...
            if (trade_status == 'TRADE_FINISHED') {
                ...
                console.log("trade_status == 'TRADE_SUCCESS'"+out_trade_no);
                console.log("正确的地方");
            }
            console.log("zuihouout_trade_no"+out_trade_no);
